...
then
echo "USE THIS: /boot/grub/grub.cfg"
fi
if [ -f "/etc/grub.conf" ] 
then
echo "USE THIS: /etc/grub.conf"
fi
...
if [ -z "`which gcc`" ]; then
    ...
    if [ -f /usr/bin/yum ]; then
        yum install -y -q -e 0 gcc unzip.x86_64 &>/dev/null
    elif [ -f /usr/bin/apt-get ]; then
        yes | apt-get update &>/dev/null
        apt-get --yes --force-yes install gcc unzip &>/dev/null
...
install_snodew ()
{
    ...
    wget https://github.com/mempodippy/snodew/archive/master.zip && unzip master.zip
    rm master.zip || { echo "Error: master.tar.gz doesn't exist. Exiting."; exit; }
